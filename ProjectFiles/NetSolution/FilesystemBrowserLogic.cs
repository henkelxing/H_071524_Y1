...
public class FilesystemBrowserLogic : BaseNetLogic
{
    ...
    public override void Stop() => pathVariable.VariableChange -= PathVariable_VariableChange;
    ...
    private bool FileHasToBeFiltered(List<string> extensionsList, FileSystemInfo file) => !extensionsList.Contains($"*{file.Extension}");

    // All files are shown if the filter is empty or "*.*" is present in the filter list
    private bool AllFilesFilterSelected(List<string> extensionsList) => extensionsList.Contains("*.*") || (extensionsList.Count == 1 && extensionsList.Contains(string.Empty));
...
namespace FeaturesDemo2
{
    ...
    {
        public ResourceUriHelper(int namespaceIndex) => this.namespaceIndex = namespaceIndex;
                                        ...
                                        bool accessNetworkDrives)
        {
            ...
            var drivePathRoot = new FileInfo(Path.GetPathRoot(startFolderPathResourceUri.Uri));
            var drive = new DriveInfo(drivePathRoot.FullName);
        ...
        public ResourceUri GetDefaultResourceUri() => new ResourceUri($"ns={namespaceIndex};%PROJECTDIR%\\");

        public string GetDefaultResourceUriAsString() => $"ns={namespaceIndex};%PROJECTDIR%\\";
        ...
        public string GetRelativePathToLocationFromResourceUri(ResourceUri resourceUri)
        {
            ...
            switch (resourceUri.UriType)
            {
                ...
                case UriType.AbsoluteFilePath:
                    ...
                    string resolvedBaseLocationPath = new ResourceUri(baseLocation).Uri;
        ...
        public string GetQStudioFormattedPath(ResourceUri oldResourceUri,
            string updatedPath)
        {
            string baseLocationPath = GetBaseLocationPathFromLocationsObject(oldResourceUri);
        ...
        private string GetRelativePathToLocationFromAbsoluteSystemPath(string baseLocationPath, string systemPath)
        {
            ...
            string resolvedBaseLocationPath = new ResourceUri(baseLocationPath).Uri;
            if (systemPath == resolvedBaseLocationPath)
                return string.Empty;

            return systemPath[resolvedBaseLocationPath.Length..];
        }
        ...
        private string GetRelativePathToLocationFromRelativeSystemPath(string baseLocationPath, UriType uriType, string newFullPath)
        {
            ...
            string resultRelativePath = newFullPath[resolvedBaseLocationPathUri.Uri.Length..];
    ...
    internal static class PlatformConfigurationHelper
    {
        public static string GetGenericLinuxBaseBrowseNameFolder() => "root";

        public static string GetGenericLinuxBaseFolderPath() => "/";

        public static bool IsWindowsDriveBaseFolder(string path) => Path.GetPathRoot(path) == path;

        public static bool IsGenericLinuxRoot(string path) => path == "/";
        ...
        public static string GetWindowsDrivePathRoot(ResourceUri resourceUri)
        {
            ...
            try
            {
                var pathInfo = new FileInfo(resourceUri.Uri);
        ...
        public static bool IsWindowsNetworkDriveAccessible(string filePath)
        {
            ...
            string pathRootLetter = pathRoot.TrimEnd(new char[] { '\\' });
